...
DISCOVERY_TEMPLATES = [
    # 周一：小红书最近 14 天热议
    "近 14 天小红书博主热议的北京宠物友好新地点（site:xiaohongshu.com 优先）",
    # 周二：小红书咖啡馆
    "{ym}小红书上被多人推荐的北京新开宠物友好咖啡馆",
    # 周三：小红书餐厅
    "{ym}小红书上被多人推荐的北京宠物友好新餐厅 / 小酒馆",
    # 周四：小红书民宿/营地
    "{ym}小红书上被多人推荐的北京宠物友好新民宿 / 营地 / 宠物乐园",
    # 周五：小红书宠物市集 / 活动
    "近 30 天小红书博主分享的北京宠物市集 / 领养日 / 嘉年华",
    # 周六：小红书周末出行
    "近 14 天小红书博主分享的北京带狗周末好去处（必须给出小红书帖子链接）",
    # 周日：小红书"打卡"专题
    "本月（{ym}）小红书「宠物友好」话题下热门的北京打卡店",
]
...
def build_discovery_prompt(title):
    return f"""你是面向养狗人的北京周末出行编辑。用 Google 搜索调研下面这个主题，**搜索时优先用 `site:xiaohongshu.com` 检索词**，让大部分结果来自小红书博主的真实分享。

主题：{title}

# 严格要求
- **来源构成**：至少 60% 的地点来自小红书帖子（其余可以来自大众点评、马蜂窝、什么值得买、澎湃等）
- 只整理近 30 天内被网友/媒体提到的地点（旧的网红店不要写）
- 每个地点必须给出最新提及的具体日期（YYYY-MM 格式）
- 每个地点行尾必须紧跟 markdown 行内链接 `([来源](URL))`，URL 必须真实
- 没有近期证据的地点直接不写
- **不要写到地点的"标语化通用描述"**（如"环境优美"），必须复述小红书博主提到的具体细节（"有专用宠物厕所"/"会送狗狗零食"等）

# 输出格式（严格两段）

## PART_A_MARKDOWN
（1500~2500 字 markdown，含至少 6 个具体地点，每条包含：地址 / 商圈、为什么宠物友好（基于博主原话）、人均花费、近期提及日期、来源链接（优先小红书原帖））
...
# 提交前自检
- pois 数组中 source_name 含"小红书"的条目应该 ≥ 60%
- 若不满足，重新搜索 site:xiaohongshu.com 加强占比
"""
